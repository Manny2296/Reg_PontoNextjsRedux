...
function AddRegPonto() {
  ...
  switch (action.type) {
    ...
    case 'UPD_REG_PONTO':
      ...
      if (action.payload != null) {
        console.log("Action updt" + _babel_runtime_corejs2_core_js_json_stringify__WEBPACK_IMPORTED_MODULE_6___default()(action.payload));

        var usertmp = _babel_runtime_corejs2_core_js_json_stringify__WEBPACK_IMPORTED_MODULE_6___default()(action.payload);

        console.log(usertmp + "-USER");
        var aupdate = [{
          "name": usertmp.name,
          "ndoc": usertmp.ndoc,
          "horaingreso": usertmp.horaingreso,
          "horasalida": usertmp.horasalida
        }];
        ...
        var usuario_a_updt = dataold.filter(function (user) {
          return user.ndoc == numerodocumento && (user.horasalida == "" || user.horasalida == "--------");
        });
        dataold[dataold.indexOf(usuario_a_updt)] = aupdate[0];
        console.log(dataold[dataold.indexOf(usuario_a_updt)]);
        nextState = {
...
//# sourceMappingURL=_app.js.efb7a0912b8ed9afc48e.hot-update.js.map